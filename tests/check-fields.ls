...
check-fields = (web3t, cb)->
    ...
    coins =
        Object.keys web3t
    return cb "Cannot find coins #{coins.join(',')}" if coins.filter(-> it not in types).length > 0
    return cb "Cannot find types" if types.filter(-> it not in coins).length > 0
    return cb "Cannot find field in definition" if coins.filter((t)-> Object.keys(web3t[t]).filter((f)-> f not in fields).length > 0 ).length > 0
