// Generated by LiveScript 1.6.0
(function(){
  var url, mainnet, testnet, type, enabled, token, color, image, usdInfo, out$ = typeof exports != 'undefined' && exports || this;
  url = 'https://royalswap.com';
  out$.mainnet = mainnet = {
    decimals: 2,
    txFee: 50,
    messagePrefix: 'Ethereum',
    mask: '0x0000000000000000000000000000000000000000',
    api: {
      provider: 'eth',
      url: url + "/fiat/explorer",
      apiUrl: url + "/fiat-token/usdt_rs",
      web3Provider: url + "/fiat-token/usdt_rs"
    }
  };
  out$.testnet = testnet = {
    disabled: true
  };
  out$.type = type = 'coin';
  out$.enabled = enabled = true;
  out$.token = token = 'usdt_rs';
  out$.color = color = '#26A17B';
  out$.image = image = "data:image/png;base64,";
  out$.usdInfo = usdInfo = "url(https://min-api.cryptocompare.com/data/pricemulti?fsyms=BTC,ETH,LTC,DASH,XEM,USDT,ETC&tsyms=USD).USDT.USD";
}).call(this);
